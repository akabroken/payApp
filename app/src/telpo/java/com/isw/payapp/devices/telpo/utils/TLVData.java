package com.isw.payapp.devices.telpo.utils;
...
public class TLVData {
    ...
    public String toString(){
        ...
        builder.append("TAG      :" + StringUtils.Int2HexString_upcase(this.Tag));
        builder.append("\nLEN      :" + this.Len);
        builder.append("\nValue    :" + StringUtils.bytesToHexString_upcase(this.Value));
    ...
    public static byte[] generateTLVArray(TLVData inTLV){
        ...
        String sTemp = StringUtils.Int2HexString(inTLV.Tag);
        sTemp += StringUtils.Int2HexString(inTLV.Len);
        if(inTLV.Len != 0) {
            sTemp += StringUtils.bytesToHexString(inTLV.Value);
        }
        Log("--- generateTLVArray --- : "+sTemp);
        byte[] ret = StringUtils.hexStringToByte(sTemp);
        return ret;
    }
    ...
    public static int PraseScript(byte[] inScript, int inScriptLen, byte[] outScript71, byte[] outScript72){
        ...
        Log("script71   :" + StringUtils.bytesToHexString_upcase(script71tmp));

        Log("script72Len:" + script72Len);
        Log("script72   :" + StringUtils.bytesToHexString_upcase(script72tmp));

        if(script71Len>0){
            outScript71 = new byte[script71Len];//无效
            System.arraycopy(script71tmp,0,outScript71,0,script71Len);
            Log("out 71:" + StringUtils.bytesToHexString_upcase(outScript71));
        }
        if(script72Len!=0){
            outScript72 = new byte[script72Len];//无效
            System.arraycopy(script72tmp,0,outScript72,0,script72Len);
            Log("out 72:" + StringUtils.bytesToHexString_upcase(outScript72));
        }
