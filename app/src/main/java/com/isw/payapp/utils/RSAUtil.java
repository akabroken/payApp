...
public class RSAUtil {
    ...
    public void generateKeyPair(int keySize) {
        try {
            ...
            Log.i("Generated RSA size: " ,""+ keySize);
        } catch (Exception e) {
    ...
    public static String decryptHexCiphertext(String hexCiphertext, RSAPrivateKey privateKey) {
        try {
            ...
            Log.d("Hex ciphertext length: " ,""+ hexCiphertext.length());

            byte[] ciphertextBytes = hexStringToByteArray(hexCiphertext);
            Log.d("Ciphertext length: " ,"" +ciphertextBytes.length);

            Cipher cipher = Cipher.getInstance(RSA_TRANSFORMATION);
            cipher.init(Cipher.DECRYPT_MODE, privateKey);

            byte[] decryptedBytes = cipher.doFinal(ciphertextBytes);
            Log.d("Decrypted byte length: " ,""+ decryptedBytes.length);
    ...
    public  String decryptWithKeyComponents(String encryptedData, String modulusBase64,
        ...
        try {
            ...
            Log.d("Decrypted Value::",bytesToHex(decryptedBytes));

            return bytesToHex(decryptedBytes);


        } catch (Exception e) {
            Log.e("key components error:" , e.getMessage());
            return null;
        }
    }
    ...
    public static String decryptHexCiphertextForIPEK(String hexCiphertext, RSAPrivateKey privateKey) {
        try {
            ...
            byte[] ciphertextBytes = hexStringToByteArray(hexCiphertext);
            Log.d("Ciphertext byte length:" , ""+ciphertextBytes.length);
            ...
            Log.d("Raw decrypted hex: " , decryptedHex);
            ...
            e.printStackTrace();
            Log.e("IPEK decryption error: " , e.getMessage());
            return null;
        }
    }
    ...
    private static String extractIPEK(byte[] decryptedData) {
        ...
        Log.d("IPEK extraction from " ,""+ decryptedData.length + " bytes");
        ...
        if (decryptedData.length >= 8) {
            ...
            if (possibleIPEK != null) {
                Log.i("IPEK","IPEK found via direct extraction: " + possibleIPEK);
                return possibleIPEK;
            }
        }
        ...
        if (asn1IPEK != null) {
            Log.i("IPEK","IPEK found via ASN.1 parsing: " + asn1IPEK);
            return asn1IPEK;
        }
        ...
        if (stringExtracted != null) {
            Log.i("IPEK","IPEK found via string extraction: " + stringExtracted);
            return stringExtracted;
        }

        Log.w("IPEK","Could not extract IPEK from decrypted data");
        return null;
    }
    ...
    private static String extractPossibleIPEK(byte[] data) {
        ...
        if (data.length == 8) {
            ...
            Log.d("ExtractPossibleIPEK","Data is exactly 8 bytes, treating as IPEK: " + ipek);
            return ipek;
        }
    ...
    private static String parseASN1ForIPEK(byte[] data) {
        try {
            ...
            if (data.length > 2 && data[0] == 0x30) {
                Log.d("ParseASN1ForIPEK","ASN.1 SEQUENCE detected, attempting parsing");
                ...
                for (int i = 0; i < data.length - 8; i++) {
                    ...
                    if (isValidIPEK(potentialKeyHex)) {
                        Log.d("ParseASN1ForIPEK","Found potential IPEK at position " + i + ": " + potentialKeyHex);
                        return potentialKeyHex;
                    }
                }
            }
        } catch (Exception e) {
            Log.w("ParseASN1ForIPEK","ASN.1 parsing failed: " + e.getMessage());
        }
        return null;
    }
    ...
    private static String extractIPEKFromString(byte[] data) {
        try {
            ...
            Log.d("ExtractIPEKFromString","Data as string: " + asString);
            ...
            if (matcher.find()) {
                ...
                Log.d("ExtractIPEKFromString","Found hex pattern in string: " + foundHex);
                if (isValidIPEK(foundHex)) {
                    return foundHex;
                }
            }
        } catch (Exception e) {
            Log.w("ExtractIPEKFromString","String extraction failed: " + e.getMessage());
        }
        return null;
    }
    ...
    private static boolean isValidIPEK(String hexString) {
        ...
        if (hexString.toUpperCase().contains("33707E4927C4A0D5")) {
            Log.i("ExtractIPEKFromString","Found expected IPEK pattern!");
            return true;
        }
