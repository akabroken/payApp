...
//            }
    ...
    public  byte[] hexStringToByte(String hex,String tag) {
        if(hex == null || hex.length()==0){
            return null;
        }
        int len = (hex.length() / 2);
    ...
    public static int bytes2int(byte[] data){
        ...
        int len = data.length;
        byte[] b =new byte[4];
        Log.d("fan", "bytes2int: "+ StringUtils.bytesToHexString(data));
        System.arraycopy(data,0, b,len>4?0:4-len, len>4?4:len);
        Log.d("fan", "bytes2int: "+ StringUtils.bytesToHexString(b));
        return   b[3] & 0xFF |
                (b[2] & 0xFF) << 8 |
                (b[1] & 0xFF) << 16 |
                (b[0] & 0xFF) << 24;
    ...
    * */
    ...
    public static String hexToAscii(String hexStr) {
        ...
        for (int i = 0; i < hexStr.length(); i += 2) {
            String hexPair = hexStr.substring(i, i + 2); // Extract two characters
            int decimal = Integer.parseInt(hexPair, 16); // Convert hex to decimal
            output.append((char) decimal); // Convert decimal to character and append
        }
