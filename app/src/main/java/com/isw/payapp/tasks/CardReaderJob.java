...
public class CardReaderJob implements Callable<String> {
    ...
    private IccCardReadImpl cardReadListener;
    //
    ...
    private GetwayProcessor purchaseProcessor;

    public CardReaderJob(Context context, PayData payData) {
...
//        }
    ...
    public int onReadCardData() {
        ...
        try {
            ...
            if (event == IC) {
                ...
                cardReadListener = new IccCardReadImpl(context, emvService, payData, event);
                ...
                ret = detectNFC();
                if (ret == 4) {
                    listener.onCardDataRead("User Canceled");
                    return FALSE;
                } else if (ret == -1003) {
                    listener.onCardDataRead("Read Timeout");
                    return FALSE;
                } else if (ret == 0) {
                    listener.onCardDataRead("NFC Card detected");
                    ret = detectCardKernel();
                    nfcCardRead = new NfcCardReadImpl(context, ret, emvService, payData);
                    nfcCardRead.readNfcCard();
                    return TRUE;
                }
            } else {
    ...
    public void onPostingPayment() throws Exception {
        ...
        String out = null;
        purchaseProcessor = new GetwayProcessor();
        ...
        if (IC == event) {
            ...
            out = purchaseProcessor.process(prePayload);
            ...
            prePayload = nfcCardRead.procPayment();
            out = purchaseProcessor.process(prePayload);
            ;
            Log.i("TESTNFC", out);
            listener.onRequestSent("Complete");
